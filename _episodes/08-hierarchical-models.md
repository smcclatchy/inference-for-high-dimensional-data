...
# Instead, please edit 08-hierarchical-models.md in _episodes_rmd/
source: Rmd
...
![Volcano plot for t-test comparing two groups. Spiked-in genes are denoted with blue. Among the rest of the genes, those with p-value < 0.01 are denoted with red.](figure/volcano-plot-1.png)
...
![p-values versus standard deviation estimates.](figure/pval_versus_sd-1.png)
...
![Illustration of how estimates shrink towards the prior expectation. Forty genes spanning the range of values were selected.](figure/shrinkage-1.png)

```
## Error in segments((tt$s^2)[idx], rep(0.1, n), ebfit$s2.post[idx], rep(0.9, : object 'ebfit' not found
```
...
```
## Error in data.frame(dm = coef(fit)[, "fac2"], p.value = ebfit$p.value[, : object 'ebfit' not found
```
...
```
## Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'rank': object 'limmares' not found
```
