...
![MA plot of simulated RNA-seq data. Replicated measurements follow a Poisson distribution.](../fig/rna_seq_simulation.png)
...
![MA plot of replicated RNA-seq data.](../fig/NA-seq_MAplot.png)
...
![Variance versus mean plot. Summaries were obtained from the RNA-seq data.](../fig/var_vs_mean.png)
...
![Palindrome count histogram.](../fig/palindrome_count_histogram.png)
...
![Likelihood versus lambda.](../fig/mle.png)
...
![Observed counts versus theoretical Poisson counts.](../fig/obs_versus_theoretical_Poisson_count.png)
...
![Histograms of biological variance and technical variance.](../fig/bio_sd_versus_tech_sd.png)
...
![Normal qq-plot for sample standard deviations.](../fig/sd_qqplot.png)
...
![Histograms of sample standard deviations and densities of estimated distributions.](../fig/modeling_variance.png)
...
![qq-plot (left) and density (right) demonstrate that model fits data well.](../fig/variance_model_fit.png)
## Exercises
...
> > `p <- 0.49`  
> > `dbinom(2,4,p=p)`  
> > `dbinom` and `pbinom` are different. `pbinom` is cumulative, whereas 
> > `dbinom` is for an individual value. Here is a demonstration. They are all 
> > equal.
> > `p <- 0.49`  
> > `dbinom(2,4,p=p)`  
> > `pbinom(2,4,p=p)`  
